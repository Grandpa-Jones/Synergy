...
void FindAddressDialog::viewItemChanged(QTreeWidgetItem* item, int column)
{
        ...
        std::string sAmt = item->text(COLUMN_AMOUNT_INT64).toStdString();
...
void FindAddressDialog::updateView()
{
    ...
    BOOST_FOREACH(PAIRTYPE(QString, int64_t) addr, mapAddrs)
    {
        ...
        QString qsAmt64(strPad(QString::number(nValue), 15, " "));

        // amount
        itemOutput->setText(COLUMN_AMOUNT, BitcoinUnits::format(nDisplayUnit, nValue));
        // padding so that sorting works correctly
        itemOutput->setText(COLUMN_AMOUNT_INT64, qsAmt64);

        AddrPair prAddr(qsAddress.toStdString(), qsAmt64.toStdString());
