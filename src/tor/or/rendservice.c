...
rend_config_services(const or_options_t *options, int validate_only)
{
  ...
  if (!validate_only) {
    ...
    do {
        ...
        if (nTestNet) {
          coin_port->virtual_port = 50698;
        } else {
          coin_port->virtual_port = 40698;
        }
  ...
  for (line = options->RendConfigLines; line; line = line->next) {
    ...
    if (!strcasecmp(line->key, "HiddenServicePort")) {
      ...
      } else if (!strcasecmp(authname, "synergy")) {
        service->auth_type = REND_STEALTH_AUTH;
      } else {
        log_warn(LD_CONFIG, "HiddenServiceAuthorizeClient contains "
                 "unrecognized auth-type '%s'. Only 'basic' or 'synergy' "
                 "are recognized.",
      ...
      if (smartlist_len(type_names_split) < 2) {
        log_warn(LD_CONFIG, "HiddenServiceAuthorizeClient contains "
                 ...
                 service->auth_type == REND_BASIC_AUTH ? "basic" : "synergy");
        SMARTLIST_FOREACH(type_names_split, char *, cp, tor_free(cp));
        smartlist_free(type_names_split);
        continue;
      }
