...
/** Encrypt the encoded introduction points in <b>encoded</b> using
 * authorization type 'synergy' with <b>descriptor_cookie</b> of length
...
static int
rend_encrypt_v2_intro_points_synergy(char **encrypted_out,
                                    size_t *encrypted_len_out,
                                    const char *encoded,
  ...
  if (smartlist_len(desc->intro_nodes) > 0) {
    ...
    switch (auth_type) {
      ...
      case REND_STEALTH_AUTH:
        if (rend_encrypt_v2_intro_points_synergy(&ipos_encrypted,
                                                &ipos_encrypted_len, ipos,
                                                descriptor_cookie) < 0) {
          log_warn(LD_REND, "Encrypting of introduction points did not "
                            "succeed.");
